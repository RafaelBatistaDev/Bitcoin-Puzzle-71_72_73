#!/usr/bin/env node

/**
 * PROJETO: BNB PUZZLE - Solver Simplificado
 * PERFORMANCE: Alta
 * AMBIENTE: Node.js v18+ (ESM nativo)
 * 
 * Funcionalidades:
 * - Importa solver modular de bnb/config/solver.js
 * - Suporta múltiplos puzzles (71, 72, 73)
 * - Gera endereços BNB sequencialmente (ou aleatório via SEARCH_MODE)
 * - Segue CryptoEngine.privkeyToAddress() de bnb/config/utils.js
 * - Consulta saldo via BNB RPC
 * - Trata rate limiting automático
 * - Salva achados em relatorio_final/saldos_encontrados.jsonl
 */

import { BnbSolver } from './bnb/config/solver.js';
import { RUNTIME_CONFIG } from './bnb/config/config.js';

console.log('\n╔════════════════════════════════════════════════════════════╗');
console.log('║  🚀 BNB PUZZLE SOLVER - Iniciando                          ║');
console.log('╚════════════════════════════════════════════════════════════╝\n');

const puzzleId = RUNTIME_CONFIG.PUZZLE_ID;
const searchMode = RUNTIME_CONFIG.SEARCH_MODE || 'sequential';

if (![71, 72, 73].includes(puzzleId)) {
  console.error('❌ Puzzle inválido! Deve ser 71, 72 ou 73');
  console.error(`   Configurado: ${puzzleId}`);
  process.exit(1);
}

console.log(`📋 Configuração:`);
console.log(`  ├─ Puzzle: BNB_PUZZLE_${puzzleId}`);
console.log(`  ├─ Modo: ${searchMode}`);
console.log(`  ├─ Batch Size: ${RUNTIME_CONFIG.BATCH_SIZE}`);
console.log(`  └─ Delay: ${RUNTIME_CONFIG.DELAY_MS}ms\n`);

const solver = new BnbSolver(puzzleId);

// Graceful shutdown
process.on('SIGINT', () => {
  console.log('\n\n⏸️  Salvando estado...');
  solver._saveState();
  console.log('✅ Estado salvo com sucesso');
  process.exit(0);
});

process.on('SIGTERM', () => {
  console.log('\n\n⏸️  Terminando gracefully...');
  solver._saveState();
  process.exit(0);
});

// Inicia busca sequencial via BnbSolver
solver.search().catch(err => {
  console.error('❌ Erro fatal:', err);
  solver._saveState();
  process.exit(1);
});
